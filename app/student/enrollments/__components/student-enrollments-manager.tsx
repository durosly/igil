...
export type StudentEnrollmentsListPayload = {
...
interface StudentEnrollmentsManagerProps {
	initialPayload: StudentEnrollmentsListPayload;
...
export default function StudentEnrollmentsManager({
	initialPayload,
	programs,
}: StudentEnrollmentsManagerProps) {
	...
	const { data } = useQuery({
		queryKey: ["student-enrollments", page, pageSize, programId, source, debouncedQ],
		queryFn: async () => {
			const { data: payload } = await axios.get<StudentEnrollmentsListPayload>(
		...
		<div className="space-y-4">
			<div className="flex flex-wrap items-end gap-3">
				...
				<div className="form-control flex-1 min-w-[200px] max-w-md">
					<label className="label py-1">
						<span className="label-text text-xs">Search program</span>
					</label>
					<input
						type="search"
			...
			<div className="overflow-x-auto">
				<table className="table table-zebra">
					...
					<tbody>
						{enrollments.map((e) => (
							<tr key={e._id}>
								<td className="font-medium">{e.programTitle}</td>
								...
								<td>
									<Link href={`/student/enrollments/${e._id}`} className="btn btn-ghost btn-xs">
										Details
									</Link>
								</td>
							</tr>
